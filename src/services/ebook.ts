...
// PDF conversion settings interface
...
// Convert ebook to PDF via Python server using Calibre
export async function convertToPdf(
  handle: FileSystemFileHandle,
  settings: ConversionSettings,
  onProgress?: (progress: number) => void
): Promise<{ success: boolean; pdfData?: ArrayBuffer; filename?: string; error?: string }> {
  try {
    const file = await handle.getFile();
    const arrayBuffer = await file.arrayBuffer();
    ...
    const response = await fetch(`${PYTHON_SERVER_URL}/convert`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        data: base64,
    ...
    const pdfBinary = atob(data.pdf_data);
    ...
    return { 
      success: false, 
      error: e instanceof Error ? e.message : 'Connection to Python server failed' 
    };
  }
}

// Download converted PDF
export function downloadPdf(pdfData: ArrayBuffer, filename: string): void {
